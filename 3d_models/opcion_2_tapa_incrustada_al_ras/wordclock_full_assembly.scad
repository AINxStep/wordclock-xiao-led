// ==============================================================================
// WORDCLOCK ESP32 - OPCION 2: VISTA GENERAL DE ENSAMBLAJE & EXPLOSIONADA (EXPLODED VIEW)
// Modelo concentrado para auditoría 3D de ajuste entre piezas (Marco 1.6 mm + Tapa Al Ras 184.4x236.4 mm)
// ==============================================================================
...
module full_wordclock_assembly() {
    // 1. Pieza 1: Marco Principal Optimizado (Z = 0)
    ...
    // 2. Pieza 2: Placa Porta-LEDs (Apoyada a Z = face_thick + baffle_h = 11.6 mm)
    if (show_led_plate == 1) {
        translate([0, 0, 11.6 + (explode_distance * 0.5)]) {
    ...
    // 3. Pieza 4: Soporte de Electrónica (Centrado a Z = 18.0 mm)
    if (show_electronics_mount == 1) {
        translate([49.0, 87.5, 18.0 + (explode_distance * 1.1)]) {
    ...
    // 4. Pieza 3: Tapa Trasera Incrustada (Montada a Z = 33.0 mm AL RAS del marco)
    // Offset XY de incrustación: (wall_thick + clearance) = 1.6 + 0.2 = 1.8 mm
    if (show_back_lid == 1) {
        translate([1.8, 1.8, 33.0 + (explode_distance * 1.8)]) {
