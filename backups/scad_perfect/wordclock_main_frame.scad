...
// Reversed matrix_text column order so text reads E S O N X L A S U N A on front view.
// ==============================================================================
...
// Dimensiones Exteriores del Marco Cuadrado
box_w       = 240.0;   // Ancho del marco exterior (mm)
box_h       = 240.0;   // Alto del marco exterior (mm)
box_d       = 38.0;    // Profundidad de la caja (mm)
wall_thick  = 3.0;     // Grosor de pared del marco exterior (mm)
...
face_thick  = 2.4;     // Grosor de la carátula frontal (mm)
baffle_h    = 15.0;    // Altura de paredes divisoras del baffle (mm)
baffle_wall = 1.6;     // Grosor de pared anti-sangrado de luz (mm)

cell_w      = led_pitch - baffle_wall; // ~11.91 mm
cell_h      = led_pitch - baffle_wall; // ~11.91 mm
...
// Posicionamiento del Bloque de 4 Puntos de Minutos (Centrado horizontalmente en X=120mm)
dots_block_cols = 4;
...
module wordclock_main_frame() {
    union() {
        difference() {
            // 1. Paredes Exteriores del Marco Cuadrado + Tapa Frontal
        ...
        translate([0, 0, face_thick]) {
            difference() {
                union() {
                    ...
                    // b) Bloque Extendido Independiente para los 4 Puntos de Minutos (Centrado en X=120mm)
        ...
        translate([wall_thick + 4, wall_thick + 4, 0])                 screw_boss(box_d, 4.5, 1.4);
        translate([box_w - wall_thick - 4, wall_thick + 4, 0])         screw_boss(box_d, 4.5, 1.4);
        translate([wall_thick + 4, box_h - wall_thick - 4, 0])         screw_boss(box_d, 4.5, 1.4);
        translate([box_w - wall_thick - 4, box_h - wall_thick - 4, 0]) screw_boss(box_d, 4.5, 1.4);
    }
}
